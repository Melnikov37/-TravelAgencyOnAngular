...
var roomTypes = null;
var transports = null;
var touristDestinations = null;
var placeDestinations = null;
...
function LoadTuorOperator(selectId) {
  var request = new XMLHttpRequest();
  request.open("GET", "/all_tour_operators/", false);
  request.send();
  ...
  selectText +=
    '<select class="form-control" name="operator" id="inputOperatorId">';
  for (i in myObj) {
    if (selectId == myObj[i]._id) {
      selectText +=
        '<option selected value="' +
        myObj[i]._id +
        '">' +
        myObj[i].tourOperatorName +
        "</option>";
    } else {
      selectText +=
        '<option value="' +
...
function LoadFoodTypes(selectId) {
  var request = new XMLHttpRequest();
  request.open("GET", "/all_food_types/", false);
  request.send();
  ...
  selectText +=
    '<select class="form-control" name="operator" id="inputFoodType">';
  for (i in myObj) {
    if (selectId == myObj[i]._id) {
      selectText +=
        '<option selected value="' +
...
function LoadRoomTypes(selectId) {
  var request = new XMLHttpRequest();
  request.open("GET", "/all_room_types/", false);
  request.send();
  ...
  selectText += '<label for="inputOperatorId">Туроператор</label>';
  selectText +=
    '<select class="form-control" name="operator" id="inputRoomType">';
  for (i in myObj) {
    if (selectId == myObj[i]._id) {
      selectText +=
        '<option selected value="' +
...
function LoadPlaceDestinations(selectID) {
  var request = new XMLHttpRequest();
  request.open("GET", "/all_place_destinations/", false);
  request.send();

  var myObj = JSON.parse(request.responseText);
  placeDestinations = myObj;
}

function LoadTransports(selectID) {
  var request = new XMLHttpRequest();
  request.open("GET", "/all_transports/", false);
  request.send();
  ...
  selectText +=
    '<select class="form-control" name="transport" id="inputTransportId">';
  for (i in myObj) {
...
function LoadTouristDestinations(selectID) {
  var request = new XMLHttpRequest();
  request.open("GET", "/all_tourist_destinations/", false);
  request.send();
  var myObj = JSON.parse(request.responseText);
  for (let id = 0; id < myObj.length; id++) {
    var request = new XMLHttpRequest();
    request.open(
      "GET",
      "/placeDestinations/" + myObj[id].placeDestinationId,
      false
    );
    request.send();
    myObj[id].placeDestination = JSON.parse(request.responseText);
  }
  touristDestinations = myObj;

  var selectText = "";
  selectText += '<label for="inputTouristDestinations">Отель</label>';
  selectText +=
    '<select class="form-control" name="touristDestinations" id="inputTouristDestinations">';
  for (i in myObj) {
    if (selectID == myObj[i].touristDestinationId) {
      if (myObj[i].numberStars != null) {
        selectText +=
          ...
          myObj[i].numberStars +
          "*, " +
          myObj[i].placeDestination.country +
          ", " +
          myObj[i].placeDestination.city +
          "</option>";
      } else {
        selectText +=
          '<option selected value="' +
...
var FlagTour = [
  ...
  [
    "Абхазия",
    "http://s1.iconbird.com/ico/1012/Flag/w64h641351178514Abkhazia.png"
  ],
  [
    "Тайланд",
    "http://www.iconsearch.ru/uploads/icons/finalflags/48x48/thailand-flag.png"
  ],
  ["Россия", "https://image.flaticon.com/icons/svg/206/206604.svg"]
];
...
function LoadTourInfo(tourId) {
  ...
  request.open(
    "GET",
    "/touristDestinations/" + tour.touristDestinationId,
    false
  );
  request.send();
  ...
  request.open(
    "GET",
    "/placeDestinations/" + touristDestination.placeDestinationId,
    false
  );
  request.send();
  ...
  tour.foodType = foodType;
  tour.roomType = roomType;
  ...
  hotelInfoHeaderHTML +=
    '<h5 class="modal-title">' + tour.touristDestination.hotelName;
  ...
  hotelInfoHeaderHTML +=
    '<span><img src="' +
    FlagTourIdent(tour.touristDestination.placeDestination.country) +
    '" width="22px" height="22px"> ' +
    tour.touristDestination.placeDestination.country +
    "</span>";
  hotelInfoHeaderHTML += "</div>";
  hotelInfoHeaderHTML += "</div>";
  ...
  photosDivHTML +=
    '<img class="d-block" src="' +
    "https://cdn.101hotels.ru/uploads/image/hotel/9595/569474.jpg" +
    '" width="350px" height="250px" alt="' +
    "asd" +
    '">';
  photosDivHTML += "</div>";
  ...
  cardHeaderTourInfoHTML +=
    '<img src="https://png.icons8.com/ios/40/000000/marker.png" width="22px" height="22px"> ' +
    tour.touristDestination.placeDestination.city;
  cardHeaderTourInfoHTML +=
    '<span class="opacity-text">, вылет из ' + tour.pointDeparture + "</span>";
  cardHeaderTourInfoHTML += "</span>";
  cardHeaderTourInfoHTML += '<hr class="hr-class">';
  cardHeaderTourInfoHTML += "<span>";
  cardHeaderTourInfoHTML +=
    '<img src="https://png.icons8.com/ios/40/000000/calendar.png" width="22px" height="22px"> на ' +
    NumberNights(arrivalDate, departureDate) +
    ", " +
    ArrivToDepartDate(arrivalDate, departureDate);
  cardHeaderTourInfoHTML += "</span>";
  document.getElementById(
    "cardHeaderTourInfo"
  ).innerHTML = cardHeaderTourInfoHTML;

  var cardBodyTourInfoHTML = "";
  cardBodyTourInfoHTML += '<p class="card-text">';
  cardBodyTourInfoHTML +=
    '<img src="https://png.icons8.com/ios/40/000000/user.png" width="22px" width="22px" height="22px" height="22px"> ' +
    NumberPerson[tour.tourNumberPersons - 1];
  cardBodyTourInfoHTML += "</p>";
  cardBodyTourInfoHTML += '<p class="card-text">';
  cardBodyTourInfoHTML +=
    '<img src="https://png.icons8.com/ios/40/000000/restaurant.png" width="22px" width="22px"> питание: ' +
    tour.foodType.foodTypeName;
  cardBodyTourInfoHTML += "</p>";
  cardBodyTourInfoHTML += '<p class="card-text">';
  cardBodyTourInfoHTML +=
    '<img src="https://png.icons8.com/ios/40/000000/empty-bed.png" width="22px" width="22px"> номера: ' +
    tour.roomType.roomTypeName;
  cardBodyTourInfoHTML += "</p>";
  cardBodyTourInfoHTML += '<p class="card-text">';
  cardBodyTourInfoHTML +=
    '<img src="https://png.icons8.com/dotty/40/000000/money.png" width="22px" width="22px"> В стоимость тура включено: авиаперелёт, трансфер, услуги гида, проживание, питание, медицинская страховка';
  cardBodyTourInfoHTML += "</p>";
  document.getElementById("cardBodyTourInfo").innerHTML = cardBodyTourInfoHTML;

  var cardCostTourInfoHTML = "";
  cardCostTourInfoHTML +=
    '<h3 class="text-center-align">' +
    parseInt(tour.tourCost * 62.0064) +
    " руб.</h3>";
  ...
  if (tour.touristDestination.hotelSite != null) {
    tabTourInfoHTML +=
      '<span class="tourInfoTitelText">Сайт отеля: </span><a href="' +
      tour.touristDestination.hotelSite +
      '">' +
      tour.touristDestination.hotelSite +
      "</a>";
    tabTourInfoHTML += "<br>";
  ...
  if (tour.touristDestination.hotelPhone != null) {
    tabTourInfoHTML +=
      '<span class="tourInfoTitelText">Телефоны: </span>' +
      tour.touristDestination.hotelPhone;
    tabTourInfoHTML += "<br>";
  }
  if (tour.touristDestination.hotelFax != null) {
    tabTourInfoHTML +=
      '<span class="tourInfoTitelText">Факс: </span>' +
      tour.touristDestination.hotelFax;
    tabTourInfoHTML += "<br>";
  }
  if (tour.touristDestination.hotelEmail != null) {
    tabTourInfoHTML +=
      '<span class="tourInfoTitelText">Email: </span>' +
      tour.touristDestination.hotelEmail;
    tabTourInfoHTML += "<br>";
  }
  if (tour.touristDestination.numberOfRooms != null) {
    tabTourInfoHTML +=
      '<span class="tourInfoTitelText">Количество номеров: </span>' +
      tour.touristDestination.numberOfRooms;
    tabTourInfoHTML += "<br>";
  }
  if (tour.touristDestination.distanceToAirport != null) {
    tabTourInfoHTML +=
      '<span class="tourInfoTitelText">Расстояние до аэропорта: </span>' +
      tour.touristDestination.distanceToAirport +
      " км";
    tabTourInfoHTML += "<br>";
  }
  tabTourInfoHTML += "</p>";
  ...
  if (tour.touristDestination.descriptionHotel != null) {
    tabTourInfoHTML += "<hr>";
    tabTourInfoHTML +=
      "<p>" + tour.touristDestination.descriptionHotel + "</p>";
  }
  document.getElementById("tabTourInfo").innerHTML = tabTourInfoHTML;

  var tabSerTourInfoHTML = "";
  let internetSerAtTours = [];
  let parkingSerAtTours = [];
  tabSerTourInfoHTML += '<div class="row">';
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/metro/40/000000/wifi.png" class="img-thumbnail" width="32px" height="32px">Интернет</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < internetSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" + internetSerAtTours[i].internetSer.internetSerName + "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/valet-parking.png" class="img-thumbnail" width="32px" height="32px">Парковка</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < parkingSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" + parkingSerAtTours[i].parkingSer.parkingSerName + "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += "</div>";
  ...
  let sportSerAtTours = [];
  let businessSerAtTours = [];
  tabSerTourInfoHTML += '<div class="row">';
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/deadlift.png" class="img-thumbnail" width="32px" height="32px">Спорт</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < sportSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" + sportSerAtTours[i].sportSer.sportSerName + "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/business.png" class="img-thumbnail" width="32px" height="32px">Бизнес-услуги</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < businessSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" + businessSerAtTours[i].businessSer.businessSerName + "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += "</div>";

  let roomFacilitiesAtTours = [];
  let foodTypeAtTours = [];
  tabSerTourInfoHTML += '<div class="row">';
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/safe.png" class="img-thumbnail" width="32px" height="32px">Удобства в номере</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < roomFacilitiesAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" +
      roomFacilitiesAtTours[i].roomFacilities.roomFacilitiesName +
      "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/metro/100/000000/food.png" class="img-thumbnail" width="32px" height="32px">Питание</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < foodTypeAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" + foodTypeAtTours[i].foodType.foodTypeName + "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += "</div>";

  let transportSerAtTours = [];
  let сleaningSerAtTours = [];
  tabSerTourInfoHTML += '<div class="row">';
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/car.png" class="img-thumbnail" width="32px" height="32px">Транспорт</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < transportSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" + transportSerAtTours[i].transportSer.transportSerName + "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/machine-wash-hot.png" class="img-thumbnail" width="32px" height="32px">Услуги по очистке одежды</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < сleaningSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" + сleaningSerAtTours[i].cleaningSer.cleaningSerName + "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += "</div>";

  let entertaimentSerAtTours = [];
  let healthAndBeautySerAtTours = [];
  tabSerTourInfoHTML += '<div class="row">';
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/billiards.png" class="img-thumbnail" width="32px" height="32px">Развлечения</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < entertaimentSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" +
      entertaimentSerAtTours[i].entertaimentSer.entertaimentSerName +
      "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/heart-health.png" class="img-thumbnail" width="32px" height="32px">Здоровье и красота</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < healthAndBeautySerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" +
      healthAndBeautySerAtTours[i].healthAndBeautySer.healthAndBeautySerName +
      "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
  ...
  let restOnWaterSerAtTours = [];
  tabSerTourInfoHTML += '<div class="row">';
  tabSerTourInfoHTML += '<div class="col">';
  tabSerTourInfoHTML +=
    '<h6><img src="https://png.icons8.com/ios/100/000000/sea-waves.png" class="img-thumbnail" width="32px" height="32px">Отдых на воде</h6>';
  tabSerTourInfoHTML += "<ul>";
  for (let i = 0; i < restOnWaterSerAtTours.length; i++) {
    tabSerTourInfoHTML +=
      "<li>" +
      restOnWaterSerAtTours[i].restOnWaterSer.restOnWaterSerName +
      "</li>";
  }
  tabSerTourInfoHTML += "</ul>";
  tabSerTourInfoHTML += "</div>";
...
function LoadTour(tourID) {
  if (tourID != null) {
    ...
    document.getElementById("inputDepartureDate").value =
      myObj.tourDepartureDate;
    document.getElementById("inputCost").value = myObj.tourCost;
    document.getElementById("inputNumberTransactions").value =
      myObj.tourNumberTransactions;
    ...
    document.getElementById("inputNumberPersons").value =
      myObj.tourNumberPerson;
    ButtonEdit(tourID);
  } else {
    LoadTouristDestinations(null);
    LoadTransports(null);
    LoadTuorOperator(null);
    LoadFoodTypes(null);
    LoadRoomTypes(null);
    ButtonAdd();
  }
...
function SearchTour() {
  HotelAndCountryAndCityNameSearch = document.getElementById(
    "hotelAndCountryAndCityNameSearch"
  ).value;
...
function DrawTour(myObj) {
  ...
  x += "<br>";
  x +=
    '<img src="https://png.icons8.com/material/50/000000/worldwide-location.png" width="18px" height="18px">';
  x +=
    '<span class="sity"> ' +
    myObj.touristDestination.placeDestination.city +
    "</span>";
  x += '<h3 class="cost">' + parseInt(myObj.tourCost * 62.0064) + " руб</h3>";
  x += "<br>";
  x +=
    '<button type="button" class="btn btn-outline-primary btn-block" data-toggle="modal" data-target="#tourInfoModal" onclick="LoadTourInfo(\'' +
    myObj._id +
    "');\">Просмотреть</button>";
  x += "</div>";
  ...
  x +=
    '<div class="align-middle d-flex flex-column bd-highlight mb-3" style="height: 100%;">';

  x += '<div class="p-2 bd-highlight">';
  x +=
    '<img src="https://png.icons8.com/android/50/000000/room.png" width="18px" height="18px">';
  x += " Вид номера: <span>" + myObj.roomType.roomTypeName + "</span>";
  x += "</div>";
  x += '<div class="p-2 bd-highlight">';
  x +=
    '<img src="https://png.icons8.com/android/50/000000/restaurant.png" width="18px" height="18px">';
  ...
  if (RoleUser == "admin") {
    ...
    x +=
      '<button class="btn btn-outline-primary dropdown-toggle" type="button" id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Действия</button>';
    x += '<div class="dropdown-menu" aria-labelledby="dropdownMenu2">';
    x +=
      '<button class="dropdown-item" type="button" data-toggle="modal" data-target="#exampleModal" onclick="LoadTour(\'' +
      myObj._id +
      "');\">Редактировать</button>";
    x +=
      '<button class="dropdown-item" type="button" onclick="DeleteTour(\'' +
      myObj._id +
      "');\">Удалить</button>";
    x += "</div>";
    x += "</div>";
...
function LoadTours() {
  ...
  x = "";
  var request = new XMLHttpRequest();
  request.open("GET", "/all_tours/", false);
  request.send();

  myObj = JSON.parse(request.responseText);
  for (var iad = 0; iad < myObj.length; iad++) {
    let request = new XMLHttpRequest();
    request.open(
      "GET",
      "/touristDestinations/" + myObj[iad].touristDestinationId,
      false
    );
    request.send();
    let touristDestination = JSON.parse(request.responseText);
    ...
    myObj[iad].foodType = foodType;
    myObj[iad].roomType = roomType;
  }
  x = "";
  if (
    HotelAndCountryAndCityNameSearch == "" &&
    ArrivalDateSearch == "" &&
    NumberNightsSearch == "0" &&
    NumberPersonSearch == "0" &&
    PointDepartureSearch == "0"
  ) {
    for (i in myObj) {
      DrawTour(myObj[i]);
    }
    ...
    if (HotelAndCountryAndCityNameSearch == "") {
      for (i in myObj) {
        ...
        if (
          СomparingDates(myObj[i].tourArrivalDate) &&
          NumberNightsSearch == NumberNights(arrivalDate, departureDate) &&
          NumberPersonSearch == myObj[i].tourNumberPerson &&
          PointDepartureSearch == myObj[i].pointDeparture
        ) {
          DrawTour(myObj[i]);
        }
      }
    ...
    for (i in myObj) {
      var arrivalDate = new Date(myObj[i].tourArrivalDate);
      var epartureDate = new Date(myObj[i].tourDepartureDate);
      if (
        (HotelAndCountryAndCityNameSearch ==
          myObj[i].touristDestination.hotelName ||
          HotelAndCountryAndCityNameSearch ==
            myObj[i].touristDestination.placeDestination.city ||
          HotelAndCountryAndCityNameSearch ==
            myObj[i].touristDestination.placeDestination.country) &&
        СomparingDates(myObj[i].tourArrivalDate) &&
        NumberNightsSearch == NumberNights(arrivalDate, departureDate) &&
        NumberPersonSearch == myObj[i].tourNumberPerson &&
        PointDepartureSearch == myObj[i].pointDeparture
  ...
  if (RoleUser == "admin") {
    var html = "";
    html +=
      '<button type="button" class="btn btn-primary justify-content-end" data-toggle="modal" data-target="#exampleModal" onclick="LoadTouristDestinations(null);LoadTuorOperator(null);LoadTransports(null);ButtonAdd();">Добавить тур</button>';
...
function GetCurrentUser() {
  ...
  SetStatus(true, "Melnikov Maksim", "admin");
  LoadTours();
  //};
  //xmlhttp.send();
}
...
function LogDelsCard(name) {
  ...
  DelsCardHTML +=
    '<input type="text" class="form-control" id="nameDelsInput" required>';
  DelsCardHTML += "</div>";
  DelsCardHTML += '<div class="col">';
  DelsCardHTML += '<label for="emailDelsInput">Email*</label>';
  DelsCardHTML +=
    '<input type="text" class="form-control" id="emailDelsInput" value="' +
    name +
    '" required>';
  DelsCardHTML += "</div>";
  DelsCardHTML += '<div class="col">';
  ...
  DelsCardHTML +=
    '<input type="text" class="form-control" id="phoneDelsInput" required>';
  DelsCardHTML += "</div>";
  DelsCardHTML += "</div>";
  DelsCardHTML += '<div class="form-group">';
  DelsCardHTML += '<label for="commentDelsInput">Ваш комментарий</label>';
  DelsCardHTML +=
    '<textarea class="form-control" id="commentDelsInput" placeholder="Не более 500 символов" rows="3"></textarea>';
  DelsCardHTML += "</div>";
  DelsCardHTML += '<div class="row">';
  DelsCardHTML += '<div class="col-5">';
  DelsCardHTML += '<p style="font-weight:bold;">';
  DelsCardHTML +=
    '<img class="leftimg" src="https://png.icons8.com/ios/100/000000/exclamation-mark.png" width="40px" height="40px">Заявка не является бронированием. Менеджер уточнит наличие тура и свяжется с вами.</p>';
  DelsCardHTML += "</div>";
  DelsCardHTML += '<div class="d-flex col justify-content-end">';
  DelsCardHTML +=
    '<button id="buttomDelsEdit" type="button" class="btn btn-success">Оставить заявку</button>';
  DelsCardHTML += "</div>";
  DelsCardHTML += "</div>";
  DelsCardHTML += "</form>";
  DelsCardHTML += "</div>";
  ...
  DelsCardHTML +=
    '<p class="small-text">Нажимая на кнопку, вы даете согласие на обработку персональных данных и соглашаетесь с <a href="">политикой конфиденциальности</a>.</p>';
  DelsCardHTML += "</div>";
...
function UnPostRatingMenu(touristDestinationsID) {
  ...
  ratingButtomDivHTML +=
    '<select id="ratingSelect" class="form-control form-control-sm">';
  ...
  ratingButtomDivHTML += '<div class="col">';
  ratingButtomDivHTML +=
    '<button id="ratingButtomSend" type="button" class="btn btn-warning btn-sm">Оставить отзыв</button>';
  ratingButtomDivHTML += "</div>";
  ratingButtomDivHTML += "</div>";
  ratingButtomDivHTML += "</div>";
  ratingButtomDivHTML += "</form>";
...
function PostRatingMenu() {
  ...
  ratingButtomDivHTML += '<div class="d-flex justify-content-end">';
  ratingButtomDivHTML +=
    '<h6 style="color:green"><img src="https://png.icons8.com/metro/100/000000/checkmark.png" width="23px" height="23px">Вы уже ставили свой отзыв</h6>';
  ratingButtomDivHTML += "</div>";
  document.getElementById("ratingButtomDiv").innerHTML = ratingButtomDivHTML;
}
...
function UnLogDelsCard() {
  ...
  DelsCardHTML +=
    '<img class="leftimg" src="https://png.icons8.com/ios/100/000000/high-priority.png" width="40px" height="40px">Для оставления заявки требуеться регистрация.</h4>';
  DelsCardHTML += "</div>";
  document.getElementById("delsForm").innerHTML = DelsCardHTML;
}
...
function UnLogMenu() {
  ...
  html +=
    '<button type="button" class="btn btn-block" data-toggle="modal" data-target="#registerModal" style="background-color: #FFFF00;"><img src="https://png.icons8.com/ultraviolet/50/000000/add-user-male.png" weight="22px" height="22px">Зарегистрироваться</button>';
  html += "</div>";
  html += '<div class="p-2 bd-highlight">';
  html +=
...
function DealsListDraw() {
  ...
  for (const i in deals) {
    if (RoleUser == "admin") {
      ...
      dealsListDivHTML += '<div class="col">';
      dealsListDivHTML +=
        "<h6>" + deals[i].tour.touristDestination.hotelName + "</h6>";
      dealsListDivHTML += "</div>";
      dealsListDivHTML += '<div class="col">';
      dealsListDivHTML +=
        deals[i].tour.touristDestination.placeDestination.country +
        ", " +
        deals[i].tour.touristDestination.placeDestination.city;
      dealsListDivHTML += "</div>";
      dealsListDivHTML += '<div class="col">';
      ...
      if (deals[i].condition.conditionName == "Обработка") {
        dealsListDivHTML +=
          ...
          deals[i].condition.conditionName +
          "</span>";
      }
